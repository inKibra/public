...
import {
  ...
  requestPermissions,
  watchPosition,
} from '@inkibra/tauri-plugin-geolocation';
import { impactFeedback } from '@inkibra/tauri-plugin-haptic-feedback';
import {
  fetchProducts,
  purchaseProduct,
  restorePurchases,
} from '@inkibra/tauri-plugin-iap';
import { showMap } from '@inkibra/tauri-plugin-map-display';
import {
  checkPermissions as checkNotificationsPermissions,
  checkRegistrationStatus,
  registerForRemoteNotifications,
  requestPermissions as requestNotificationsPermissions,
  watchNotifications,
...
function render() {
  const outlet = document.getElementById(
    'inkibra-tauri-plugin-testbed-app-outlet',
  );
  if (!outlet) {
  ...
  ReactDOM.createRoot(outlet).render(
    <React.StrictMode>
      ...
      <button
        onClick={async () => {
          const checkNotificationsPermissionsResponse =
            await checkNotificationsPermissions();
          console.log(
            'checkNotificationsPermissionsResponse',
            checkNotificationsPermissionsResponse,
          );
        }}
      >
      ...
      <button
        onClick={async () => {
          const requestNotificationsPermissionsResponse =
            await requestNotificationsPermissions();
          console.log(
            'requestNotificationsPermissionsResponse',
            requestNotificationsPermissionsResponse,
          );
        }}
      >
      ...
      <button
        onClick={async () => {
          const registerForRemoteNotificationsResponse =
            await registerForRemoteNotifications();
          console.log(
            'registerForRemoteNotificationsResponse',
            registerForRemoteNotificationsResponse,
          );
        }}
      >
      ...
      <button
        onClick={async () => {
          const checkRegistrationStatusResponse =
