...
### Bundle Manifest (Recommended)

The plugin also supports a bundle manifest that includes JavaScript and CSS URLs
directly (ESM-friendly). Use this for modern builds with CSS extraction:

```ts
import { prepareBundle, startBundle } from '@inkibra/tauri-plugin-ota';

const info = await prepareBundle(
  'https://your-cdn.com/.ios-app-bundle-manifest.json',
  {
    devManifestPrompt: true,
    devManifestPrimarySources: [
      {
        label: 'prod',
        manifestUrl:
          'https://ota-manifest.example.com/.ios-app-bundle-manifest.json',
      },
      {
        label: 'staging',
        manifestUrl:
          'https://ota-manifest-staging.example.com/.ios-app-bundle-manifest.json',
      },
    ],
  },
);
console.log('OTA bundle manifest', info);
await startBundle();
```

In development mode, `prepare` and `prepareBundle` can prompt before fetch so
you can choose from:
- named primary domains (for example `prod` and `staging`)
- your previous manual overrides (scrollable list)
- your last used manual override
- a newly entered manual override

You configure the named primary domains in app setup via
`devManifestPrimarySources`.

Each primary source can be a full URL or just a host. When only a host is
provided, the plugin keeps the original manifest path from `manifestUrl`.

This works for both legacy OTA manifests (`prepare`) and bundle manifests
(`prepareBundle`).
...
type PrepareOptions = {
  ...
  devManifestPrimarySources?: Array<{
    label: string;
    manifestUrl: string;
  }>;
